...
lamda = [0.05124, 0.1954];  %Setting Thresholds

x_train = [ZigZag10;ZigZag20]; % training data
x_vali = D2ZigZag25;  % test data
x = x_train;
x = wdenoise(x);      %denoise
u = x(:,9)*pi/180;
va = x(:,7);
x = [va x(:,6) x(:,1)*pi/180]; 

dt=0.2;   %In order to align the prediction process with the time in the sensor data
...
lambda_vec = [10,lamda2,lamda3];  %Set sparse knob Adjusts according to Bayesian optimization results
...
yout = selfpooldatalist(Xi,LibraryType);  %display the results
%% Make a forecast
n=4;
Nvar = 3;
datapre = x_vali;
...
psi_pre = data_pre(1,10);  %Storing bow angle information
x_p(1,:)=xv;
...
%% Plot--------------------------------------------------------------------------
rmse_v = sqrt(mean((x_p(:,2)-data_pre(:,6)).^2))   %Calculate the root mean square error
