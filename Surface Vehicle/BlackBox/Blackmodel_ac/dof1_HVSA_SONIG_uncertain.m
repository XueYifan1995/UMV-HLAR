...
sonig.addIIPDistance = 0.3; % This is the distance (normalized with respect to the length scales) above which new inducing input points are added.
su = hyp.sx(1);
sx= [hyp.sx(2)];
...
jointCov = [su.^2,zeros(1,2);zeros(2,1),blkdiag(diag(sx.^2),diag(sx.^2))]; % This will contain the covariance matrix of the SONIG input which we're currently applying.
for i = 2:nm
	...
	jointMean = [u(i-1);jointMean(3);xm(:,i)]; % We shift the mean matrix one further.
	jointCov = [su.^2,zeros(1,2);zeros(2,1),[jointCov(2,2),zeros(1,1);zeros(1,1),diag(sx.^2)]]; % We shift the covariance matrix one further.
	jointDist = createDistribution(jointMean, jointCov);
	inputDist = getSubDistribution(jointDist, 1:2);
	outputDist = getSubDistribution(jointDist, 3);
	...
	xpo(:,i-1:i) = reshape(jointPost.mean(2:3),[1,2]);
	stds = sqrt(diag(jointCov)');
	ustd(:,i-1) = stds(1);
	xstd(:,i-1:i) = reshape(stds(2:3),[1,2]);
end
...
for i=1:1:m
    ...
    [TEMP_a,TEMP_sm,Temp_U,rci]=Infante_HVSA_1dof_unCertain(time,U,Spost,x,sy,sonig,i)  ;
    Spost = rci;
    %Euler
    x= x + dt.*TEMP_a;
    sy =TEMP_sm;
...
subplot(212),plot(T,psi,'linewidth',1.6);hold on
plot(T,duo);hold off
