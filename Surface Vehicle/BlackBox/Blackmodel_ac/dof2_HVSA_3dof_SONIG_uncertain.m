...
sonig.addIIPDistance = 0.05; % This is the distance (normalized with respect to the length scales) above which new inducing input points are added.
t1 = toc;  %time for training
%% Add new points
su = hyp.sx(1);
sx= [hyp.sx(2);hyp.sx(3)];
...
jointCov = [su.^2,zeros(1,4);zeros(4,1),blkdiag(diag(sx.^2),diag(sx.^2))]; % This will contain the covariance matrix of the SONIG input which we're currently applying.
for i = 2:nm
	...
	jointMean = [u(i-1);jointMean(4:5);xm(:,i)]; % We shift the mean matrix one further.
	jointCov = [su.^2,zeros(1,4);zeros(4,1),[jointCov(4:5,4:5),zeros(2,2);zeros(2,2),diag(sx.^2)]]; % We shift the covariance matrix one further.
	jointDist = createDistribution(jointMean, jointCov);
	inputDist = getSubDistribution(jointDist, 1:3);
	outputDist = getSubDistribution(jointDist, 4:5);
	...
	xpo(:,i-1:i) = reshape(jointPost.mean(2:5),[2,2]);
	stds = sqrt(diag(jointCov)');
	ustd(:,i-1) = stds(1);
	xstd(:,i-1:i) = reshape(stds(2:5),[2,2]);
end
...
for i=1:1:m
    ...
    T(i,1)=t;
    time = t;
    % one Multi-output SONIG
    [TEMP_a,TEMP_sm,Temp_U,vci,rci]=Infante_SONIG_2dof_HVSA(time,U,Spost,x,sy,sonig,i)  ;
    Spost = [vci;rci];
    %Euler
    x= x + dt.*TEMP_a;
    sy =TEMP_sm;
...
subplot(313),plot(T,psi,'linewidth',1.6);hold on
plot(T,duo);hold off
